...
export default function HirePassFeed({
  ...
  // ✅ show/hide TikTok embed (only one creator on screen anyway)
        ...
        >
          ...
          <div className="mt-3 flex gap-3">
            <button
              ...
              className="w-full rounded-2xl bg-zinc-800 py-3 text-center text-sm font-bold text-white"
            >
            ...
            <a
              ...
              rel="noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="w-full rounded-2xl bg-white py-3 text-center text-sm font-bold text-black"
            >
              Open TikTok
            </a>
          </div>
          ...
          {showVideo ? (
            <div
              className="mt-4 overflow-hidden rounded-2xl bg-black/30 ring-1 ring-white/10"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-3 text-xs text-gray-400">
                If the embed doesn’t load, tap “Open TikTok”.
              </div>
              <div className="px-2 pb-3">
                {/* using tiktok_link as video URL for now */}
               <TikTokEmbed videoUrl={current.tiktok_link} />
              </div>
            </div>
          ) : (
            <div className="mt-4 rounded-2xl bg-zinc-800/60 p-5 text-sm text-gray-300">
              <div className="font-semibold text-white">About</div>
              <div className="mt-1">{current.notes ?? "—"}</div>
          ...
          {result ? (
            ...
            <div className="mt-3 text-center text-xs text-gray-500">
              {/* Showing creators in JSON order (newest first). */}
            </div>
          )}
